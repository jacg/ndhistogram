...
# the time of writing this is nixos-21.11.

{
  py ? "39" # To override the default python version:  nix-shell shell.nix --argstr py 38
}:
...
let
  ...
  python-with-all-my-packages = (python.withPackages (ps: [
    # Do we really need these for a Rust project?
    ps.isort
    ps.mypy
    ps.black
  ]));
